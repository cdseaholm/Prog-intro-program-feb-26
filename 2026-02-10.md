...
My notes: 
  ...
  - Building apis within .NET/Visual Studio
  1. Have an AppHost within Solution
  2. Have a FOLDERNAME.Api like Gateway.Api. 
    a. Optional: Within this folder, set properties/launchSettings.json so you have an https url
      with the correct settings
  3. In main folder (or .cs file at root)
    a. Create public record (response structure)
      i. In here this is where you set the params you need (name, description, etc)
    b. Then create a static class wrapped around an async Task for the response based on params
  4. Once API running (ctrl + f5), can send a request