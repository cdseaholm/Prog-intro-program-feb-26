...
- Attribute sometime need to be on class, this one is on method
    - Example: 'Fact' is the attribute and it tells the method is going to do something for fact
    - From library: "Attribute that is applied to a method to indicate that it is a fact that should be run by the test runner. It can also be extended to support a customized definition of a test method."
- Assert.Equal(expected, actual)
- For ints, without "M" after it "5000M", it means int32, with M means decimal